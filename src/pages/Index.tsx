...
type AppState = 'landing' | 'auth' | 'training' | 'dashboard' | 'profile' | 'certifications' | 'rewards' | 'green-champion';
type UserType = 'waste-collector' | 'student' | 'community-leader' | 'employee';
...
const Index = () => {
  ...
  const handleNavigate = (path: string) => {
    switch (path) {
      case '/':
        ...
        break;
      case '/learning':
        // This will be handled by the Learning component
        break;
      case '/profile':
        setAppState('profile');
        break;
  ...
  const handleBack = () => {
    if (appState === 'auth') {
      setAppState('landing');
      setUserType(null);
    } else if (['profile', 'certifications', 'rewards', 'green-champion'].includes(appState)) {
      setAppState('dashboard');
    }
  };

  const showNavbar = ['dashboard', 'profile', 'certifications', 'rewards', 'green-champion'].includes(appState);
    ...
    <div className="min-h-screen bg-background">
      ...
      {(() => {
        switch (appState) {
          ...
          case 'certifications':
            return <Certifications userData={userData} onBack={handleBack} />;
